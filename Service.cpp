...
/**
 * function creates account , ask if user wants to generate password
 */
...
/**
 *  function creates password and ask user for date
 * @param generatePassword state if user wants to generate password
 * @return password
 */
Account Service::createAccount(bool generatePassword) {
    ...
    do {
        ...
        if (cat < 0 || cat >= categories.size())
            std::cout << "\nwybrano zla liczbe! (wybierz liczbe z zakresu 0-" << categories.size() - 1 << ")"
                      << std::endl;
...
/**
 * function asks user what account should be edited , and what params should be edited and changes em
 */
...
/**
 * function asks user what account should be deleted ,and deletes it
 */
...
/**
 * function shows all accounts
 */
void Service::showAll() {
    std::vector<Account> accounts = accountRepository.getAll();
    if (accounts.empty())
        std::cout << "nie ma jeszcze zadnego  konta \n";
    for (Account a: accounts) {
        std::cout << "\t Haslo :\n";
        std::cout << a.toString();
    }
...
/**
 * function asks user what category should be deleted , and deletes it
 */
void Service::deleteCategory() {
    ...
    if (allCat.empty()){
        std::cout<< "nie ma jeszcze zadnej kategori !\n";
        return;
    }
    ...
    std::cout << "kategoria " << toDelete << " usunieta \n";
...
/**
 * function asks user for param , and shows accounts that match given param
 */
...
/**
 * function shows sorted list of accounts
 */
